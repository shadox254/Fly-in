...
class Hub():
    ...
    def __init__(self, name: str, x: int, y: int, connections: dict[str, int],
        ...
        Args:
            ...
            connections (dict[str, int]): Dictionary where keys are neighbor
                names and values are the path costs.
        ...
        if type not in zonetype_list:
            zonetype_str = (', '.join(map(str, zonetype_list[:-1])) +
                            ' or ' + zonetype_list[-1])
            raise TypeError(f'Error, invalid zone type: {type}, '
                            f'type must be {zonetype_str}')
